...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    computed: {
      ...
      // songReady为false时，功能上通过ready()函数实现不能快速点击，在样式也要实现
      disableCls() {
    ...
    created() {
      // 在created里面定义，是因为不需要去添加getter和setter
      this.touch = {}
    },
    methods: {
      updateTime(e) {
        // e.target.currentTime是当前播放时间，是一个可读写的属性
      ...
      next() {
        ...
        if (this.playlist.length === 1) {
          ...
          // 保证在暂停状态切换时，可以进行播放，CD可转，播放按钮变化；否则歌曲已经播放，上述两个没有相应变化
          if (!this.playing) {
            this.togglePlaying()
          }
        }
        // 点击后将songReady置为false
      ...
      middleTouchMove(e) {
        ...
        // 因为歌词列表有纵向滚动的scroll，所以当纵向滚动大于横向滚动时，就不切换
        ...
        // this.$refs.lyricList是一个vue组件，不能直接访问DOM，要先访问$el
      ...
      _getPosAndScale() {
        ...
        // Normal是目标点，mini相对与normal的横向是负的
    ...
    watch: {
      currentSong(newSong, oldSong) {
        ...
        // 不加延时的话，DOM的src还没更新就去play，play会报错
        // 处理微信后台运行切前台问题，换为setTimeout
        /* this.$nextTick(() => {
         this.$refs.audio.play()
         this.getLyric()
         }) */
